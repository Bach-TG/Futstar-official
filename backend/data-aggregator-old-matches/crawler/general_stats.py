...
from mongo.schemas import MatchGeneralStats
...
async def get_stats_data(driver: webdriver.Chrome, wait: WebDriverWait[webdriver.Chrome], title: str, time: str):
    ...
    # with open(f"{title}_stats_at_{time}.json", "w", encoding="utf-8") as f:

    #     json.dump(stats_data_dict, f, ensure_ascii=False, indent=4)
    try:
        general_stats_record = MatchGeneralStats(
            time_in_match=time,
            possession=(stats_data_dict["possession"][0], stats_data_dict["possession"][1]),
            possession_lost_defensive=(stats_data_dict["possession_lost_defensive"][0], stats_data_dict["possession_lost_defensive"][1]),
            possession_lost_midfield=(stats_data_dict["possession_lost_midfield"][0], stats_data_dict["possession_lost_midfield"][1]),
            duels=(stats_data_dict["duels"][0], stats_data_dict["duels"][1]),
            duels_accuracy=(stats_data_dict["duels_accuracy"][0], stats_data_dict["duels_accuracy"][1]),
            aerial_duels=(stats_data_dict["aerial_duels"][0], stats_data_dict["aerial_duels"][1]),
            aerial_duels_accuracy=(stats_data_dict["aerial_duels_accuracy"][0], stats_data_dict["aerial_duels_accuracy"][1]),
            dribbles_success=(stats_data_dict["dribbles_success"][0], stats_data_dict["dribbles_success"][1]),
            fouls_won=(stats_data_dict["fouls_won"][0], stats_data_dict["fouls_won"][1]),
            offsides=(stats_data_dict["offsides"][0], stats_data_dict["offsides"][1]),
            corners_won=(stats_data_dict["corners_won"][0], stats_data_dict["corners_won"][1]),
        )
        await general_stats_record.insert()
        print(f"Đã lưu dữ liệu thống kê chung vào MongoDB cho thời điểm '{time}'.")
    except Exception as e:
        print(f"LỖI khi lưu dữ liệu thống kê chung vào MongoDB: {e}")
...
async def main():
    ...
    try:
        ...
        try:
            print("\n--- BẮT ĐẦU: Click tab 'General' ---")
            # Chờ cho đến khi tab có thể click được
            tab = wait.until(
                EC.element_to_be_clickable((By.LINK_TEXT, "General"))
            )
            ...
            print(f"LỖI: Không thể click vào tab 'General'. {e}")
            raise # Dừng script nếu không click được
        print("--- KẾT THÚC: Click tab 'General' ---\n")
        # --- KẾT THÚC BƯỚC MỚI ---

        await hold_and_slowly_drag_end(driver=driver, wait=wait, title="General")
