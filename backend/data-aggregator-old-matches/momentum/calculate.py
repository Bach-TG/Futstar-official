...
async def get_stats_from_db() -> tuple[list[TeamMomentumStats], list[TeamMomentumStats]]:
    ...
    for stat in all_attack_stats:
        ...
        home_team_stats = MomentumUsedStats(
            ...
            penalty_area_entries=distribution_map[time_in_match].penalty_area_entries[0],
            open_play_crosses=distribution_map[time_in_match].open_play_crosses[0],
            final_thirds_entries=distribution_map[time_in_match].final_thirds_entries[0],
            dribbles_success=general_map[time_in_match].dribbles_success[0],
            corners_won=general_map[time_in_match].corners_won[0],
            possession_lost_defensive=general_map[time_in_match].possession_lost_defensive[0],
            possession_lost_midfield=general_map[time_in_match].possession_lost_midfield[0],
            offsides=general_map[time_in_match].offsides[0],
            ball_recoveries_attacking=defence_map[time_in_match].ball_recoveries_attacking[0],
            interceptions=defence_map[time_in_match].interceptions[0],
            ball_recoveries_midfield=defence_map[time_in_match].ball_recoveries_midfield[0],
            ball_recoveries_defensive=defence_map[time_in_match].ball_recoveries_defensive[0],
        ...
        away_team_stats = MomentumUsedStats(
            ...
            penalty_area_entries=distribution_map[time_in_match].penalty_area_entries[1],
            open_play_crosses=distribution_map[time_in_match].open_play_crosses[1],
            final_thirds_entries=distribution_map[time_in_match].final_thirds_entries[1],
            dribbles_success=general_map[time_in_match].dribbles_success[1],
            corners_won=general_map[time_in_match].corners_won[1],
            possession_lost_defensive=general_map[time_in_match].possession_lost_defensive[1],
            possession_lost_midfield=general_map[time_in_match].possession_lost_midfield[1],
            offsides=general_map[time_in_match].offsides[1],
            ball_recoveries_attacking=defence_map[time_in_match].ball_recoveries_attacking[1],
            interceptions=defence_map[time_in_match].interceptions[1],
            ball_recoveries_midfield=defence_map[time_in_match].ball_recoveries_midfield[1],
            ball_recoveries_defensive=defence_map[time_in_match].ball_recoveries_defensive[1],
        ...
        home_team_full_time_stats.append(TeamMomentumStats(
            time_in_match=stat.time_in_match,
            stats=home_team_stats
        ))
        away_team_full_time_stats.append(TeamMomentumStats(
            time_in_match=stat.time_in_match,
...
async def calculate_difference_stats(previous_stats: MomentumUsedStats, current_stats: MomentumUsedStats) -> MomentumUsedStats:
    return MomentumUsedStats(
        ...
        shots_inside_box=current_stats.shots_inside_box - previous_stats.shots_inside_box,
        shots_headed=current_stats.shots_headed - previous_stats.shots_headed,
        shots_outside_box=current_stats.shots_outside_box - previous_stats.shots_outside_box,
        ...
        penalty_area_entries=current_stats.penalty_area_entries - previous_stats.penalty_area_entries,
        open_play_crosses=current_stats.open_play_crosses - previous_stats.open_play_crosses,
        final_thirds_entries=current_stats.final_thirds_entries - previous_stats.final_thirds_entries,
        dribbles_success=current_stats.dribbles_success - previous_stats.dribbles_success,
        corners_won=current_stats.corners_won - previous_stats.corners_won,
        possession_lost_defensive=current_stats.possession_lost_defensive - previous_stats.possession_lost_defensive,
        possession_lost_midfield=current_stats.possession_lost_midfield - previous_stats.possession_lost_midfield,
        offsides=current_stats.offsides - previous_stats.offsides,
        ball_recoveries_attacking=current_stats.ball_recoveries_attacking - previous_stats.ball_recoveries_attacking,
        interceptions=current_stats.interceptions - previous_stats.interceptions,
        ball_recoveries_midfield=current_stats.ball_recoveries_midfield - previous_stats.ball_recoveries_midfield,
        ball_recoveries_defensive=current_stats.ball_recoveries_defensive - previous_stats.ball_recoveries_defensive,
...
async def calculate_threat_score(action_list: list[TeamMomentumStats], lambda_value: LambdaEnumForDuration, current_time: str) -> float:
    threat_score = 100.0
    for action in action_list:
        ...
        time_diff = await calculate_difference_time(previous_time=action.time_in_match, current_time=current_time)
        decay_factor = math.exp(-lambda_value.value * time_diff)
        threat_score += threat_contribution * decay_factor
    return threat_score


async def calculate_momentum_stats(team_stats: list[TeamMomentumStats], lambda_value: LambdaEnumForDuration) -> list[TeamThreatScore]:
    ...
    for s in team_stats:
        ...
        stat = s.stats
        actions_list.append(TeamMomentumStats(time_in_match=time_in_match, stats=await calculate_difference_stats(previous_stats=previous_momentum_stats, current_stats=stat)))
        if time_in_match.find("+") != -1:
            first_half_end_time = time_in_match
        while len(actions_list) > 1 and (await calculate_difference_time(actions_list[0].time_in_match, time_in_match, first_half_end_time)) > 600:
            actions_list.pop(0)
        threat_score = await calculate_threat_score(actions_list, lambda_value, time_in_match)
        threat_scores.append(TeamThreatScore(
...
async def calculate_full_momentum():
    ...
    home_team_momentum = await calculate_momentum_stats(home_team_stats, LambdaEnumForDuration.FIVE_MINUTES)
    print("Home Team momentum calculation complete")
    print("Calculating Away Team momentum")
    away_team_momentum = await calculate_momentum_stats(away_team_stats, LambdaEnumForDuration.FIVE_MINUTES)
...
if __name__ == "__main__":
    ...
    for hm, am in zip(home_team_momentum, away_team_momentum):
        momentum_score = hm.threat_score / (hm.threat_score + am.threat_score) * 100 if (hm.threat_score + am.threat_score) != 0 else 50.0
        print(f"At time {hm.time_in_match}, Home Team Threat Score: {hm.threat_score}, Away Team Threat Score: {am.threat_score}, Momentum Index: {momentum_score}")

        if hm.time_in_match.startswith("45:") and momentum_index[-1]["time_in_match"].startswith("45+"):
