...
class UserMetadataDB(Document):
    ...
    bonus_points: float | int | None = Field(default=0, description="Bonus points awarded to the user")
    ...
    class Settings:
        ...
        validate_on_save = True
        indexes = ["wallet_address", "id", "email", "telegram_handle", "twitter_handle", "discord_handle"]
...
class PositionHistoryDB(Document):
    ...
    bonus_points_rewarded: float | int | None = Field(default=0, description="Bonus points rewarded for this position")
    class Settings:
        name = "position_history"  # Collection name in MongoDB
        validate_on_save = True
        indexes = ["id", "user_id", "match_id"]
...
class MissionMetadataDB(Document):
    ...
    reward_points: int = Field(default=..., description="Reward points for completing the mission")
    type: Literal["daily", "weekly", "monthly", "special", "social", "beginner", "challenge", "retention"] = Field(default=..., description="Type of the mission")    
    is_active: bool = Field(default=True, description="Indicates if the mission is currently active")
    expiration_date: datetime | None = Field(default=None, description="Expiration date of the mission")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Timestamp when the mission was created")
    number_of_steps: int = Field(default=1, description="Number of steps required to complete the mission")

    class Settings:
        name = "mission_metadata"  # Collection name in MongoDB
        validate_on_save = True
        indexes = ["mission_name", "id"]


class UserMissionDB(Document):
    id: UUID = Field(default_factory=uuid4, description="Unique identifier for the user mission")
    user_id: str = Field(default=..., description="Unique identifier for the user")
    mission_id: str = Field(default=..., description="Unique identifier for the mission")
    ...
    last_updated: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Last updated timestamp for the mission progress")
    is_claimed: bool = Field(default=False, description="Indicates if the mission reward has been claimed by the user")
    number_of_steps_completed: int = Field(default=0, description="Number of steps completed by the user in the mission")
