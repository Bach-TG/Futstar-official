...
from mongo.schemas import MatchDistributionStats
...
async def get_stats_data(driver: webdriver.Chrome, wait: WebDriverWait[webdriver.Chrome], title: str, time: str):
    ...
    try:
        distribution_stats_record = MatchDistributionStats(
            time_in_match=time,
            passes=(stats_data_dict["passes"][0], stats_data_dict["passes"][1]),
            passes_long=(stats_data_dict["passes_long"][0], stats_data_dict["passes_long"][1]),
            passes_accuracy=(stats_data_dict["passes_accuracy"][0], stats_data_dict["passes_accuracy"][1]),
            passes_final_third=(stats_data_dict["passes_final_third"][0], stats_data_dict["passes_final_third"][1]),
            passes_final_third_accuracy=(stats_data_dict["passes_final_third_accuracy"][0], stats_data_dict["passes_final_third_accuracy"][1]),
            passes_forward=(stats_data_dict["passes_forward"][0], stats_data_dict["passes_forward"][1]),
            passes_forward_accuracy=(stats_data_dict["passes_forward_accuracy"][0], stats_data_dict["passes_forward_accuracy"][1]),
            passes_long_proportion=(stats_data_dict["passes_long_proportion"][0], stats_data_dict["passes_long_proportion"][1]),
            passes_opponents_half=(stats_data_dict["passes_opponents_half"][0], stats_data_dict["passes_opponents_half"][1]),
            passes_opponents_half_accuracy=(stats_data_dict["passes_opponents_half_accuracy"][0], stats_data_dict["passes_opponents_half_accuracy"][1]),
            open_play_crosses=(stats_data_dict["open_play_crosses"][0], stats_data_dict["open_play_crosses"][1]),
            final_thirds_entries=(stats_data_dict["final_thirds_entries"][0], stats_data_dict["final_thirds_entries"][1]),
            crosses=(stats_data_dict["crosses"][0], stats_data_dict["crosses"][1]),
            crosses_accuracy=(stats_data_dict["crosses_accuracy"][0], stats_data_dict["crosses_accuracy"][1]),
            penalty_area_entries=(stats_data_dict["penalty_area_entries"][0], stats_data_dict["penalty_area_entries"][1]),
            through_balls=(stats_data_dict["through_balls"][0], stats_data_dict["through_balls"][1]),
        )
        await distribution_stats_record.insert()
        print(f"Đã lưu dữ liệu thống kê phân phối time: {time} vào cơ sở dữ liệu MongoDB.")
    except Exception as e:
        print(f"LỖI khi lưu MatchDistributionStats cho thời điểm '{time}': {e}")
...
async def main():
    ...
    try:
        ...
        try:
            print("\n--- BẮT ĐẦU: Click tab 'Distribution' ---")
            # Chờ cho đến khi tab có thể click được
            tab = wait.until(
                EC.element_to_be_clickable((By.LINK_TEXT, "Distribution"))
            )
            ...
            print(f"LỖI: Không thể click vào tab 'Distribution'. {e}")
            raise # Dừng script nếu không click được
        print("--- KẾT THÚC: Click tab 'Distribution' ---\n")
        # --- KẾT THÚC BƯỚC MỚI ---

        await hold_and_slowly_drag_end(driver=driver, wait=wait, title="Distribution")
