...
from mongo.schemas import MatchDefenceStats
...
async def get_stats_data(driver: webdriver.Chrome, wait: WebDriverWait[webdriver.Chrome], title: str, time: str):
    ...
    try:
        defence_stats_record = MatchDefenceStats(
            time_in_match=time,
            tackles=(stats_data_dict["tackles"][0], stats_data_dict["tackles"][1]),
            tackles_accuracy=(stats_data_dict["tackles_accuracy"][0], stats_data_dict["tackles_accuracy"][1]),
            clearances=(stats_data_dict["clearances"][0], stats_data_dict["clearances"][1]),
            interceptions=(stats_data_dict["interceptions"][0], stats_data_dict["interceptions"][1]),
            ball_recoveries=(stats_data_dict["ball_recoveries"][0], stats_data_dict["ball_recoveries"][1]),
            ball_recoveries_attacking=(stats_data_dict["ball_recoveries_attacking"][0], stats_data_dict["ball_recoveries_attacking"][1]),
            ball_recoveries_attacking_accuracy=(stats_data_dict["ball_recoveries_attacking_accuracy"][0], stats_data_dict["ball_recoveries_attacking_accuracy"][1]),
            ball_recoveries_defensive=(stats_data_dict["ball_recoveries_defensive"][0], stats_data_dict["ball_recoveries_defensive"][1]),
            ball_recoveries_midfield=(stats_data_dict["ball_recoveries_midfield"][0], stats_data_dict["ball_recoveries_midfield"][1]),
        )
        await defence_stats_record.insert()
        print(f"Đã lưu dữ liệu thống kê phòng ngự time: {time} vào cơ sở dữ liệu MongoDB.")
    except Exception as e:
        print(f"LỖI khi lưu MatchDefenceStats cho thời điểm '{time}': {e}")
...
async def main():
    ...
    try:
        ...
        try:
            print("\n--- BẮT ĐẦU: Click tab 'Defence' ---")
            # Chờ cho đến khi tab có thể click được
            tab = wait.until(
                EC.element_to_be_clickable((By.LINK_TEXT, "Defence"))
            )
            ...
            print(f"LỖI: Không thể click vào tab 'Defence'. {e}")
            raise # Dừng script nếu không click được
        print("--- KẾT THÚC: Click tab 'Defence' ---\n")
        # --- KẾT THÚC BƯỚC MỚI ---

        await hold_and_slowly_drag_end(driver=driver, wait=wait, title="Defence")
